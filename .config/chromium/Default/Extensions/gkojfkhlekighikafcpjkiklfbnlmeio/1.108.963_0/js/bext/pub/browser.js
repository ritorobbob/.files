...
define(['jquery', '/util/etask.js', '/util/zerr.js', '/bext/pub/msg.js',
    '/bext/pub/backbone.js', '/bext/pub/chrome.js', 'underscore',
    '/util/version_util.js', '/bext/pub/transport.js', '/util/user_agent.js',
    '/util/util.js'],
    function($, etask, zerr, be_msg, be_backbone, be_chrome, _, version_util,
    be_transport, user_agent, zutil){
// XXX bahaa: rm
var chrome = window.chrome;
var is_popup = window.is_popup, is_tpopup = window.is_tpopup;
var use_msg = !chrome || is_tpopup;
var use_backbone_over_msg = use_msg || !chrome;
...
E.init = function(opt){
    ...
    var t = is_tpopup && zutil.get(window, 'hola.tpopup_opt.browser_have');
    E.have = t ? t : chrome ? chrome_have() : firefox_have();
    opt = opt||{};
...
function get_transport(){
    ...
    return be_transport.firefox_bg_to_addon(window.top, window.top);
}

function vcmp(v){ return version_util.cmp(zconf.ZON_VERSION, v); }
...
function chrome_have(){
    ...
    if (chrome.webRequest)
    {
        ...
        if (chrome.webRequest.onCompleted)
            have['web_request.on_body_received'] = true;
        if (chrome.webRequest.handlerBehaviorChanged)
            have['web_request.handler_behavior_changed'] = true;
        if (chrome.webRequest.onErrorOccurred)
            have['web_request.on_error_occurred'] = true;
        if (chrome.webRequest.onAuthRequired)
            have['web_request.on_auth_required'] = true;
    }
    if (chrome.storage)
    {
        if (chrome.storage.sync)
            have['storage.sync'] = true;
    }
    ...
    if (chrome.cookies)
    {
        ...
        have['cookies.get_all'] = true;
        have['cookies.remove'] = true;
    }
...
function firefox_have(){
    ...
    // plugin was removed from local code. was disabled long before
    if (vcmp('1.68.502')>=0)
        have['be.plugin'] = false;
    return have;
}
...
E.tabs = {
    ...
    is_connected: function(tab_id){
        return !!bg.be_bg_main.be_browser.msg.tab_connections[tab_id]; },
};
E.windows = {
...
E.web_request = {
    ...
    on_body_received: {
        add_listener: function(cb, opt, extra){
            E.rpc.add_listener('webRequest', 'onCompleted', cb, opt, extra);
        },
        del_listener: function(cb){
            E.rpc.del_listener('webRequest', 'onCompleted', cb); },
    },
    on_error_occurred: {
        add_listener: function(cb, opt, extra){
            E.rpc.add_listener('webRequest', 'onErrorOccurred', cb, opt,
                extra);
        },
        del_listener: function(cb){
            E.rpc.del_listener('webRequest', 'onErrorOccurred', cb); },
    },
    handler_behavior_changed: function(cb){
        E.rpc.call_api('webRequest', '', 'handlerBehaviorChanged', [cb]);
    }
};
E.management = {
    get_all: function(cb){
...
E.cookies = {
    ...
    get_all: function(details, cb){
        E.rpc.call_api('cookies', '', 'getAll', [details], cb); },
    remove: function(details, cb){
        E.rpc.call_api('cookies', '', 'remove', [details], cb); }
};
