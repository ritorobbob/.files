...
define(['jquery', 'underscore', '/bext/pub/backbone.js', '/util/etask.js',
    '/util/zerr.js', '/bext/pub/util.js', '/bext/pub/browser.js',
    '/util/date.js', '/util/array.js', '/bext/pub/lib.js',
    '/util/storage.js', '/util/escape.js', '/bext/vpn/pub/ajax.js',
    '/util/version_util.js', 'be_ver', '/util/rate_limit.js',
    '/bext/vpn/pub/svc.js', '/util/hash.js', '/www/util/pub/storage/www.js',
    '/util/user_agent.js'],
...
function _plugin_event_cb(e, ret){
	...
	E.sp.spawn(etask([function(){
	    ...
	    return this.loop(etask.sleep(30*date.ms.SEC));
	}]));
	break;
...
function setup_url(info, version){
    ...
    return zescape.uri(url,
        {cors_ff: 1, arch: x64 ? 'x64' : undefined, ver: version});
}
...
function validate_setup(version, filename, len, sha1, perr){
    ...
    return etask([function(){
        ...
        if (_len==-1)
        {
            ...
            return this.return('setup not found');
        }
        if (_len!=len)
        {
            if (perr)
            ...
            return this.return('setup invalid size '+
                (_len>len ? 'big' : 'small'));
        }
        ...
        if (_sha1!=sha1)
        {
            ...
            return this.return('setup invalid sha1');
        }
        return null;
    }]);
}
...
function do_download_setup(url, info, path, exe, version, force){
	...
	return xhr = $.ajax({type: 'HEAD', url: url+'?cors_ff=2',
        ...
        return this.throw('validate download invalid response '+url+' '+
            xhr.status+' '+len);
    }, function try_catch$(e){
	return xhr = $.ajax({url: url+'.sha1?cors_ff=2',
	    timeout: 20*date.ms.SEC});
    }, function(e){
        if (this.error)
        {
            return this.throw('validate download failed sha1 '+url+'.sha1 '+
                xhr.status);
        }
...
function download_setup(info, version, force){
    ...
    return etask([function try_catch$(){
        return ajax.json({url: url, retry: 2});
    }, function try_catch$(e){
        link = e;
        if (this.error || !link || !link.url || !link.exe)
            return this.throw('invalid response or no url/exe '+this.error);
        ...
        return this.return(etask.err('be.download: '+err));
    }, function(e){
        be_lib.ok('setup_download_'+(e ? 'valid' : 'ok'),
            url+' '+(Date.now()-ts)+'ms');
        return cb_apply(B.be.plugin, '.get_info', []);
    }, function catch$(err){
        throw 'plugin.get_info: '+err;
    }]);
}
...
function firefox_init(plugin_ver){
    ...
    return etask([function try_catch$(){
        ...
        if (this.error && this.error!='already inited')
            return this.return(etask.err('plugin.init: '+this.error));
        ...
        if (!E.plugin_info)
        {
            ...
            return this.return(etask.err('plugin.init: missing plugin_info'));
        }
        ...
        if (!E.plugin_info.setup_exec)
            return this.throw('missing setup exec');
        ...
        if (this.error)
        {
            ...
            {
                retry = true;
                return this.goto('download');
            }
            return this.return(etask.err('plugin.exec: '+this.error));
        }
        ...
        if (this.error)
            return this.return(etask.err('plugin.get_info: '+this.error));
        E.plugin_info = e;
        if (!(e = validate_plugin(E.plugin_info, plugin_ver)))
            return this.return();
        zerr.notice(e);
        return this.return(etask.err('validate_plugin: '+e));
    }]);
}
...
function chrome_init(){
    ...
    return etask([function try_catch$(){
        if (!zp || !zp.inited)
            return this.return(etask.err(!zp ? 'zp null' : 'zp.inited null'));
        return zp.inited();
    }, function(ret){
        if (this.error || ret<0)
            return this.return(etask.err('zp.inited() failed'));
...
E.init = function(plugin_ver){
    if (E.inited)
	return;
    E.dll_have_status = version_util.cmp(be_util.version(),
        '1.4.764')>=0;
...
function wait_for(opt){
    ...
    }, function finally$(){ E.off('change:'+opt.path, this.change_cb); }]);
}
...
var zp_call = E.zp_call = function(func/*, ...*/){
    var zp = E.zp, args = array.slice(arguments, 1);
...
function create_link(){
	...
	if (tries<50)
	    return this.goto('loop', etask.sleep(100));
        if (tries==50)
	    return do_kill();
...
E.start = function(first_run){
    ...
    if (!chrome)
    {
        ...
        {
            return etask([function(){
                this.alarm(10000, {throw: 'timeout'});
...
function main_init(){
    ...
    etask([function(){
        ...
        if (+user_agent.guess_browser().version<43 ||
            err!='plugin.init: no plugin')
        {
	...
	var l, _this = this;
        ...
        ts = Date.now();
        return this.goto('on', plugin_enable());
    }, function catch$(err){
	fail++;
	...
	var l, _this = this;
        ...
        ts = Date.now();
        return this.goto('off', plugin_disable());
    }, function catch$(err){
...
E.main_uninit = function(){
    ...
    return etask([function(){
        ...
        if (version_util.cmp(be_util.version(), '1.2.439')>=0)
            return;
...
function plugin_enable(){
	...
	if (E.get('running')) /* XXX bahaa: consider perr */
        ...
        return etask([function check(){
            if (!be_bg_main.get('enabled'))
                return this.return(etask.err('canceled'));
            if (be_bg_main.get('ext.slave'))
                return this.return(etask.err('slave extension'));
            if (!E.get('running') && !svc_wait)
                return this.return(etask.err('plugin crashed'));
            ...
            return this.goto('check', etask.sleep(500));
        }]);
    }, function catch$(e){
