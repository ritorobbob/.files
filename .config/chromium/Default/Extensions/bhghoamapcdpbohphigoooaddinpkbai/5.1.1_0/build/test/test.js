...
function test() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (testCaseIndex % 2 === 0) {
            clear();
            yield set(cases[Math.floor(testCaseIndex / 2)].data);
        }
        if (document.getElementsByTagName('iframe') &&
            ...
            document.getElementsByTagName('iframe')[0].onload = () => {
                document.getElementsByTagName('iframe')[0].contentWindow.addEventListener('unhandledrejection', event => {
...
function showTestResult() {
    ...
    for (let i = 0; i < testRes.length; i++) {
        ...
        el.innerHTML = `<td style="vertical-align: text-top; width: 50px; color: ${testRes[i].pass ? 'green' :
            'red'}">[${testRes[i].pass ? 'Pass' : 'Fail'}]</td>`;
        el.innerHTML +=
            `<td><h3 style="margin: 0">${cases[Math.floor(i / 2)].name}${i % 2 === 1 ? ' (Reopen)' :
                ''}</h3>${testRes[i].error}<br><pre><code>${testResData[i]}</code></pre><br></td>`;
