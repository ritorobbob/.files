...
function add_vstat(root_url, src, url, tab_url, status, content_length,
    content_type, transfer_encoding, completed)
{
    var key = root_url && src && root_url!=src ? root_url+'..'+src : root_url;
    if (!vstat.stats)
        vstat.stats = {};
    var vs = vstat.stats[key] = vstat.stats[key]||{ln_h: 0, ln_b: 0, ch: 0,
        ev: 0, ct: content_type, st: {}, feat: {}};
    vstat.stats[key].feat = vstat_feat[root_url];
    if (!root_url)
    {
        vs.url = url;
        vs.tab_url = tab_url;
...
function add_vstat_feat(url, key, val){
    vstat_feat[url] = vstat_feat[url]||{};
    vstat_feat[url][key] = val;
}

function vstat_hdr(tab_url, initiator, url, hdrs, status, from_cache,
    completed)
{
    ...
    if (url.includes('babator.com'))
        add_vstat_feat(root_url, 'bbtr', {url: url, tab_url: tab_url});
    ...
    add_vstat(root_url, src, url, tab_url, status, content_length,
        content_type, transfer_encoding, completed);
}
...
function on_body_received(details){
    if (E.vstat_hdr)
    {
        vstat_hdr(E.get_url(details.tabId), details.initiator, details.url,
