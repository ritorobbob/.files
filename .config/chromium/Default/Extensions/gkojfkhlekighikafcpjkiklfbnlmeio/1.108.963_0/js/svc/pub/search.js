...
var deps = ['jquery', 'backbone', '/util/escape.js', '/util/url.js',
    ...
    '/util/array.js', '/util/string.js'];
if (window.is_popup)
    deps.push('/bext/pub/locale.js');
else
    deps.push(null, '/www/hola/pub/be.js', '/www/hola/pub/ui.js');
define(deps, function($, Backbone, zescape, zurl, typeahead, user_agent, _,
    etask, array, string, T, be, ui){
if (!window.is_popup)
...
function suggest_html(s){
    ...
    +(window.is_popup ? 'http:' : location.protocol||'')
...
E.filter = Backbone.View.extend({
    ...
    update: _.debounce(function(options){
        ...
        if (list)
        {
            ...
            if (selector)
            {
                ...
                search_text = string.trunc(search_text, 25);
    ...
    on_keydown: function(e){
        ...
        switch (e.which)
        {
        case 0xd: e.preventDefault(); break;
        case 38:
        case 40:
            e.preventDefault();
            var key = e.which;
...
search_index.prototype.update = function(text, item){
    ...
    function add_level(level, text_array){
        ...
        if (level[val])
            array.add_elm(level[val].nodes, item);
        else
