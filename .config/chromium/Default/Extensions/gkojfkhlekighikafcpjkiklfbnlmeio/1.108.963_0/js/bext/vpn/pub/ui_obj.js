...
// XXX arik/alexeym: unite www/hola/pub/ui.js and bext/vpn/pub/ui_obj.js
...
E.trial_ended = Backbone.View.extend({
    className: 'trial_ended',
    template: '<div class=icon></div>'+
    '<div class=header>'+T('Free trial ended!')+'</div>'+
    '<a href="https://hola.org/premium?ref=holaext-trial-ended" '+
    'target=_blank class=subscribe>'+T('Register to premium')+'</a>'+
    '<a class=cancel>'+T('No, thanks')+'</a>',
    events: {
        'click .subscribe': '_on_subscribe',
        'click .cancel': '_on_cancel',
    },
    render: function(){
        be_popup_lib.perr_ok({id: 'trial_ended_view'});
        this.$el.html(this.template);
        return this;
    },
    close: function(src, period){
        this.options.close({
            root_url: 'all',
            period: period,
            src: src,
    ...
    _on_subscribe: function(){
        be_popup_lib.perr_ok({id: 'trial_ended_click_subscribe'});
        this.close('subscribe', '1d');

    },
    _on_cancel: function(){
        be_popup_lib.perr_ok({id: 'trial_ended_click_cancel'});
        this.close('cancel', 'never');
    },
});
