...
define(['/bext/pub/browser.js', '/bext/pub/backbone.js', '/util/etask.js',
    '/bext/pub/lib.js', '/bext/pub/ext.js', '/bext/vpn/pub/defines.js',
    '/util/version_util.js', '/bext/pub/util.js', 'underscore',
    '/util/storage.js', '/util/date.js', '/util/zerr.js', 'jquery',
    '/svc/hola/pub/svc_ipc.js'],
    function(B, be_backbone, etask, be_lib, be_ext, be_defines,
    version_util, be_util, _, storage, date, zerr, $, svc_ipc){
var chrome = window.chrome, zconf = window.zon_config, conf = window.conf;
...
E.init = function(){
    // XXX bahaa FF_WEBEXT: disabled for webextension as svc denies the request
    // which is sent without Origin header
    if (!conf.firefox_web_ext)
        E.svc_monitor.init();
};

E.uninit = function(){ E.svc_monitor.uninit(); };
...
EE.init = function(){
    ...
    etask('svc_init', [function wait(info){
        ...
        waiting = false;
        return this.goto('wait', info && info.svc_result || E.update_info());
    }]);
    inited = true;
};

EE.continue = function(info){
    if (monitor && waiting)
        monitor.continue(info);
};
...
E.update_info = function(){
    return etask('update_info', [function(){
        ...
        E.safe_set(change);
        E.svc_monitor.continue({svc_result: info});
        return info;
    }, function catch$(){
        E.safe_set({
            info: null,
