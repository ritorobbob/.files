// LICENSE_CODE ZON
'use strict'; /*jslint node:true, browser:true*//*global browser,alert*/
(function(){
var define;
...
// used to debug bext pac script locally, don't remove as it was hard to find
// this recipe
function log(){
    var args = Array.from(arguments).map(function(o){
        return JSON.stringify(o); });
    var text = new Date().toISOString()+' '+args.join(' ');
    // that's the only way to get logs from pac script in FF
    // console.log and alert are not working
    // https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/proxy/register#Communicating_with_PAC_files
    if (typeof browser!='undefined' && browser.runtime)
        browser.runtime.sendMessage({id: 'pac_log', data: text});
    // that's the way to log messages in Chrome
    // https://serverfault.com/a/738871
...
function local_hola_cb(url){
    ...
    {
        entry = g_pac_engine.local_redir[set] = {
            proxy: proxy,
            count: 0,
        };
    }
