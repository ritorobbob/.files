...
function init() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const authenticator = yield ui.load(className)
            .load(i18n)
            .load(menu)
            .load(info)
            .load(passphrase)
        ...
        const backupReminder = setInterval(() => {
            ...
            else if (clientTime - localStorage.lastRemindingBackupTime >= 30 ||
                clientTime - localStorage.lastRemindingBackupTime < 0) {
                // backup to Dropbox
                if (authenticator.dropboxToken) {
                    chrome.permissions.contains({ origins: ['https://*.dropboxapi.com/*'] }, (hasPermission) => __awaiter(this, void 0, void 0, function* () {
                        if (hasPermission) {
                            try {
                                const dropbox = new Dropbox();
                                const res = yield dropbox.upload(authenticator.encryption);
                                if (res) {
...
if (navigator.userAgent.indexOf('Edge') !== -1) {
    ...
    chrome.permissions.contains({ origins: ['https://www.google.com/'] }, (hasPermission) => {
        if (hasPermission) {
            syncTimeWithGoogle();
        }
    });
